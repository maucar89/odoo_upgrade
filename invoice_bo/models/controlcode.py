...
class ControlCode(object):
    ...
    def generate(self, number, amount_total, **kwargs):
        ...
        if not self.validate_amount(amount_total):
            raise InvalidParam('Invalid amount %s' % amount_total)
        ...
        ## step 1
        ##
        verhoeff5 = self._step1_verhoeff_5digits(number, nit, date_invoice, amount_total)
        ...
        for s1,s2 in zip((auth, number, nit, date_invoice, amount_total), strings):
    ...
    #
    def _step1_verhoeff_5digits(self, number, nit, date_invoice, amount_total):
        # dummy verhoeff numbers
        data = (number, nit, date_invoice, amount_total)
...
#            raise InvalidParam('Invalid auth %s' % auth)
    ...
    def set_amount(self, amount_total):
        if self.validate_amount(amount_total):
            self._amount = amount_total
        else:
            raise InvalidParam('Invalid amount %s' % amount_total)
        return self
    ...
    def validate_amount(self, amount_total):
        return isinstance(amount_total, (int,float,long)) and amount_total > 0
...
#VARIABLES NEEDED FOR THE PROCESS TO WORK: Next we detail each of them
    ...
    control_code = cc.generate(data['number'], data['amount_total'])
